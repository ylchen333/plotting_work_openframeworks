...
//    refImg.load("moldref.png");
    ...
    refPix = refImg.getPixels();
    strkWidth = 1;
    
    f = 0.0962; //0.047; //0.04;
    k = 0.05560; //0.062; //0.06;
    r = 0.01;
    dt = 0.01;
    du = 1.0;
    dv = 0.5;
...
//                double sourcePxF = (double) refPix[idx+1];
//                double sourcePxK = (double) refPix[idx];
...
//                int angle = ofMap(sourcePx, 0, 255, 0, 180);
//                feed += ofMap(sourcePxF^2, 0, 222, -0.03, 0.06);  //+ k/sin(sourcePxF/66)
...
//                kill = ofMap(y, 1, gridH, k-0.02, k+0.02);
                feed = ofMap(collatz, 1, gridH*3+1, f-0.01, f+0.06);
//                feed += ofMap(ofNoise(sourcePxF), 0.0, 1.0, -0.001, 0.02);
                
//                feed += ofRandom(-0.5, 0.5) * ofNoise(sourcePxF);
                kill = ofMap(ofDist(gridW/2, gridH/2, (float)x, (float)y), 0, ofDist(0.0, 0.0, gridW/2, gridH/2), k, k+0.01);
...
//        capture =false;
    ...
    string strkWStr = "(press and drag mouse to draw)(change with mn) strkWidth: " + ofToString(strkWidth);
...
//            restart reaction
       ...
       case ('i'):{
                exportImg.allocate(gridW, gridH, OF_IMAGE_COLOR);
                string imgName = "rdc_import_" + ofToString(ofGetFrameNum()) +
                + ".jpg";
                exportImg.grabScreen(0, 0, gridW, gridH);
                exportImg.save(imgName);
...
//                cout << ithPoly << endl;
    ...
    //        mySvgExporter1->addPolyline(aPoly, aColor);
